...
> ⚠️ **V5.2:** 7. neuronų tinklas · 8. taksistas · 9. aura — **✅ inline SVG (Higgsfield nebereikia).**
> Briefai palikti — jei kada atsiras kreditų, tikras GIF vis tiek perrašys sceną per `GIFMAP`.
...
7. ✅ **S5 neuronų tinklas** — *inline SVG (Higgsfield nebereikia)* — glowing neural network learning, signals traveling between nodes, red on white
8. ✅ **S12 taksistas** — *inline SVG (Higgsfield nebereikia)* — London black cab driver, city map growing/glowing inside his transparent head, brain lighting up
9. ✅ S11 aura battle — *inline SVG (Higgsfield nebereikia)* — stylized street crowd circle, kid doing confident dance move, floating "+1000 AURA"
...
13. ✅ S10 brain rot ×3 — *inline SVG (Higgsfield nebereikia)* — scrollinimas stiklinėm akim · laikrodis sukasi · smegenys užmiega (galima ir Higgsfield stilizuotai)
...
### 5 smegenų priešai (S8) — ✅ visi 5 inline SVG (Higgsfield nebereikia)

Briefai lieka kaip specifikacija būsimam patobulinimui.
...
## V5.2 — 11 gyvų SVG scenų vietoj Higgsfield GIF'ų (2026-09-01)

Higgsfield be kreditų, tad šie 11 `data-brief` raktų dabar yra **inline animuoti SVG**, ne placeholder'iai:

| raktas | scena |
|---|---|
| `priesas-miegas` | telefonas lovoje naktį · laikrodis 23:00 → 00:00 → 01:00 · akys užsimerkia, ratilai tamsėja |
| `priesas-sofa` | figūra įsmunka į sofą ir persilieja į bulvę · dulkės kyla |
| `priesas-kuras` | traškučiai → pilkas rūkas užgesina švytinčias smegenis · vandens lašas jas vėl uždega |
| `priesas-draugai` | vaikas ekrano šviesoje · pilki draugai · pakėlus akis — nusispalvina raudonai |
| `priesas-scroll` ⭐ | begalinė juosta telefone · nykštys braukia · galva pamažu žalsvėja (BOSAS) |
| `rot1` | trumpų video srautas skrieja aukštyn · stiklinės akys su atspindžiais |
| `rot2` | rodyklės skrieja, žmogus sustingęs · „-1 VAL." nuplaukia |
| `rot3` | smegenys fotelyje užminga, švytėjimas gęsta, zzz |
| `aura` | gatvės ratas · šokio judesys · „+1000 AURA" kyla |
| `taxi` | Londono kebas · vairuotojo profilis su augančiu gatvių žemėlapiu galvoje |
| `neural` | mazgai + keliaujantys impulsai + storėjančios jungtys (raudona ant balto) |

**Kaip veikia (`slot()` prioritetas): `GIFMAP` > `SVGMAP` > `IMGMAP`.**
Nieko trinti nereikia — įrašius tikrą GIF į `GIFMAP`, jis automatiškai perrašo inline sceną.
Konsolėje `listGifSlots()` dabar rodo ir `🎬 SVG`.

Technika (ta pati kaip V5 evoliucijos piktogramų): CSS `@keyframes` ant SVG grupių,
`transform-box:fill-box` + `transform-origin` galūnių sukiniams, be SMIL, be JS ciklų.
Lėtos 3–4 s kilpos, `prefers-reduced-motion` gerbiamas, jokio mirgėjimo.
Neaktyvios skaidrės scenos sustabdomos (`animation-play-state:paused`).

**Vis dar laukia komercinių klipų (nepaliesta):** `g2` · `quest` · `hum1-4` · `rayyan` · `gorilla` · `cubism` · `laser`.
